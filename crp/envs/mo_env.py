...
from .VVR_Simulator import VVR_Simulator
...
class MultiObjectiveEnv(object):
    ...
    def reset(self, env_name=None):
        ...
        self.env.reset()
    def mask(self, env_name=None):
        ''' reset the enviroment '''
        return self.env.get_action_out_mask()
    def observe(self):
        ''' reset the enviroment '''
    ...
    def step(self, action, step=0):
        ''' process one step transition (s, a) -> s'
            return (s', r, terminal)
        '''
        return self.env.step(action, step=step)
...
if __name__ == "__main__":
    ...
    while not terminal:
        ...
        mask=dst_env.mask()
        action = np.argmax(np.random.rand(dst_env.action_spec[2][1])*mask.numpy())
        next_state, reward, terminal = dst_env.step(action)
        print("s:", state.shape, "\ta:", action, "\ts':", next_state, "\tr:", reward)
